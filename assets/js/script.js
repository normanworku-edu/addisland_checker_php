...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Even listner for changing the region and input type selection
    const regionSelect = document.getElementById("region");
    const inputTypeSelect = document.getElementById("input_type");

    if (regionSelect) {
        regionSelect.addEventListener("change", updateInputField);
    }

    if (inputTypeSelect) {
        inputTypeSelect.addEventListener("change", updateInputField);
    }
    ...
    form.addEventListener('submit', function (event) {
        ...
        fetch('', { // Make sure this is the correct URL for your processing script
            ...
            .then(data => {
                ...
                const coordinateTableContainer = document.getElementById('coordinates-table-container');
                try {
                    let tableRows = '';
                    for (let i = 0; i < lats.length; i++) {
                        tableRows += `<tr><td class='text-nowrap'>${lats[i]}</td><td class='text-nowrap'>${lons[i]}</td></tr>`;
                    }
                    ...
                    coordinateTableContainer.innerHTML = coordinatesTable;

                }
                catch {
                    coordinateTableContainer.innerHTML = '';

                }


                const validityTextContainer = document.getElementById('validity-text-container');
                let validityMessage;
                ...
                if (isSuccess === true) {
                    ...
                    showMap(mapDataGiven); // Re-initialize map after adding/replacing

                } else {
                    validityMessage = '<div class="alert alert-danger">❌ Invalid Input: Failed to retrieve coordinates.</div>';

                    showMap();
                }
                validityTextContainer.innerHTML = validityMessage;
